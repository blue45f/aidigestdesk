  ...
  return (
    <div style={{ marginTop: 12, borderTop: `1px solid ${theme.border}`, paddingTop: 12 }}>
      ...
      <form style={{ display: 'flex', gap: 8, marginTop: 10 }}
        onSubmit={(e) => { e.preventDefault(); if (body.trim()) { addComment({ postId, author: nickname, body: body.trim() }); setBody(''); } }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <form style={{ display: 'flex', gap: 8 }}
        onSubmit={(e) => { e.preventDefault(); if (channel && draft.trim()) { addPost({ channelId: channel.id, author: nickname, body: draft.trim() }); setDraft(''); } }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <form style={{ ...card, display: 'flex', flexDirection: 'column', gap: 8 }}
        onSubmit={(e) => { e.preventDefault(); if (title.trim() && body.trim()) { addBoardPost({ category: writeCat, title: title.trim(), body: body.trim(), author: nickname }); setTitle(''); setBody(''); setCat('전체'); } }}>
      ...
      {joined && (
        <div style={{ marginTop: 12, borderTop: `1px solid ${theme.border}`, paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <form style={{ display: 'flex', flexDirection: 'column', gap: 8 }}
            onSubmit={(e) => { e.preventDefault(); if (title.trim() && body.trim()) { addBoardPost({ category: cafeCat, title: title.trim(), body: body.trim(), author: nickname }); setTitle(''); setBody(''); } }}>
